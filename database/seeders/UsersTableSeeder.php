...
class UsersTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        // Customers
        Customer::factory()->count((int)$count)->create()->each(function ($user) use ($faker, $allWarehouses) {
        ...
        // Suppliers
        Supplier::factory()->count((int)$count)->create()->each(function ($user) use ($faker, $allWarehouses) {
            foreach ($allWarehouses as $allWarehouse) {
                UserDetails::factory()->create([
                    'warehouse_id' => $allWarehouse,
                    'user_id' => $user->id,
