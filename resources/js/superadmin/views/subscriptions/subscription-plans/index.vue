<template>
    <SuperAdminPageHeader>
        <template #header>
            <a-page-header :title="$t(`menu.subscription_plans`)" class="p-0" />
        </template>
        <template #breadcrumb>
...
export default {
    ...
    setup() {
        ...
        const setUrlData = () => {
            ...
            crudVariables.langKey.value = "subscription_plans";
            crudVariables.initData.value = { ...initData };
            crudVariables.formData.value = { ...initData };
        };

        const trailPlanSetting = () => {
            loadingTrial.value = true;

            // For Trial Plan
            axiosAdmin.get("superadmin/trial-plan").then((response) => {
                crudVariables.editItem(response.data.plan);
