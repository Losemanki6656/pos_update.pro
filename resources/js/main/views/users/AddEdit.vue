        ...
        <a-form layout="vertical">
            <a-row>
                <a-col :xs="24" :sm="24" :md="6" :lg="6">
                    <a-form-item :label="$t('user.profile_image')" name="profile_image"
                        :help="rules.profile_image ? rules.profile_image.message : null"
                        :validateStatus="rules.profile_image ? 'error' : null">
                        <Upload :formData="formData" folder="user" imageField="profile_image" @onFileUploaded="(file) => {
                ...
                <a-col :xs="24" :sm="24" :md="18" :lg="18">
                    <a-row :gutter="16" v-if="warehouseVisible">
                        <a-col :xs="24" :sm="24" :md="24" :lg="24">
                            <a-form-item name="warehouse_id" :help="rules.warehouse_id ? rules.warehouse_id.message : null
                                " :validateStatus="rules.warehouse_id ? 'error' : null" class="required">
                                <template #label>
                                    {{ $t("warehouse.warehouse") }}
                                </template>
                                <span v-if="permsArray.includes('admin')" style="display: flex">
                                    <a-select v-if="formData.user_type == 'staff_members'"
                                        v-model:value="formData.warehouses" :placeholder="$t('common.select_default_text', [
                                            $t('warehouse.warehouse'),
                                        ])
                                            " mode="multiple" @change="warehousesChanged">
                                        <a-select-option v-for="warehouse in warehouses" :key="warehouse.xid"
                                            :value="warehouse.xid">
                                            {{ warehouse.name }}
                                        </a-select-option>
                                    </a-select>
                                    <a-select v-else v-model:value="formData.warehouse_id" :placeholder="$t('common.select_default_text', [
                                        $t('warehouse.warehouse'),
                                    ])
                                        ">
                                        <a-select-option v-for="warehouse in warehouses" :key="warehouse.xid"
                                ...
                                <span v-else>
                                    <a-select :value="selectedWarehouse.xid" :placeholder="$t('common.select_default_text', [
                                        $t('warehouse.warehouse'),
                                    ])
                                        " disabled>
                    ...
                    <a-row :gutter="16" v-if="formData.user_type == 'staff_members' &&
                        ...
                        <a-col :xs="24" :sm="24" :md="24" :lg="24">
                            <a-form-item :label="$t('user.role')" name="role_id"
                                ...
                                :validateStatus="rules.role_id ? 'error' : null" class="required">
                                <span style="display: flex">
                                    <a-select v-model:value="formData.role_id" :placeholder="$t('common.select_default_text', [
                                        $t('user.role'),
                                    ])
                                        " :allowClear="true" @change="roleChanged">
                                        <a-select-option v-for="role in roles" :key="role.xid" :value="role.xid"
                                            :title="role.name">
                                            {{ role.display_name }}
                                        </a-select-option>
                                    </a-select>
                    ...
                    <a-row :gutter="16">
                        ...
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                            <a-form-item :label="$t('user.phone')" name="phone"
                                ...
                                <a-input v-model:value="formData.phone" v-mask="maskPhone" :value="maskValue" :placeholder="$t('common.placeholder_default_text', [
                                    $t('user.phone'),
                                ])
                                    " />
                            </a-form-item>
                    ...
                    <a-row :gutter="16">
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                            <a-form-item :label="$t('user.email')" name="email"
                                ...
                                :validateStatus="rules.email ? 'error' : null" :class="{
                        ...
                        <a-col :xs="24" :sm="24" :md="12" :lg="12">
                            <a-form-item :label="$t('user.status')" name="status"
                                ...
                                <a-select v-model:value="formData.status" :placeholder="$t('common.select_default_text', [
                                    $t('user.status'),
                                ])
                                    ">
                    ...
                    <a-row v-if="formData.user_type == 'staff_members' ||
                        ...
                        <a-col :span="24">
                            <a-form-item :label="$t('user.password')" name="password"
                                ...
                                :validateStatus="rules.password ? 'error' : null" :class="formData.user_type == 'staff_members'
                                    ? 'required'
                                    : ''
                                    ">
                                <a-input-password v-model:value="formData.password" :placeholder="$t('common.placeholder_default_text', [
                                    $t('user.password'),
                                ])
                                    " />
                                <small v-if="addEditType == 'edit'" class="small-text-message">
                    ...
                    <a-row :gutter="16" v-if="formData.user_type != 'staff_members'">
                        <a-col :xs="24" :sm="24" :md="24" :lg="24">
                            <a-form-item :label="$t('user.tax_number')" name="tax_number"
                                :help="rules.tax_number ? rules.tax_number.message : null"
                                :validateStatus="rules.tax_number ? 'error' : null">
                                <a-input v-model:value="formData.tax_number" :placeholder="$t('common.placeholder_default_text', [
                                    $t('user.tax_number'),
                                ])
                                    " />
                            </a-form-item>
                    ...
                    <a-row :gutter="16" v-if="formData.user_type != 'staff_members'">
                        <a-col :xs="24" :sm="24" :md="24" :lg="24">
                            <a-form-item :label="$t('user.opening_balance')" name="opening_balance" :help="rules.opening_balance
                                ? rules.opening_balance.message
                                : null
                                " :validateStatus="rules.opening_balance ? 'error' : null">
                                <a-input v-model:value="formData.opening_balance" placeholder="0">
                                    <template #prefix>
                                        {{ appSetting.currency.symbol }}
                                    </template>
                                    <template #addonAfter>
                                        <a-select v-model:value="formData.opening_balance_type" style="width: 100px">
            ...
            <a-row :gutter="16" v-if="formData.user_type != 'staff_members'">
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                    <a-form-item :label="$t('user.credit_period')" name="credit_period"
                        :help="rules.credit_period ? rules.credit_period.message : null"
                        :validateStatus="rules.credit_period ? 'error' : null">
                        <a-input-number v-model:value="formData.credit_period" placeholder="0"
                            :addon-after="$t('user.days')" type="number" :precision="0" :style="{ width: '100%' }" />
                    </a-form-item>
                </a-col>
                <a-col :xs="24" :sm="24" :md="12" :lg="12">
                    <a-form-item :label="$t('user.credit_limit')" name="credit_limit"
                        :help="rules.credit_limit ? rules.credit_limit.message : null"
                        :validateStatus="rules.credit_limit ? 'error' : null">
                        <a-input v-model:value="formData.credit_limit" placeholder="0"
            ...
            <a-row :gutter="16">
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    <a-form-item :label="formData.user_type == 'staff_members'
                        ? $t('user.address')
                        : $t('user.billing_address')
                        " name="address" :help="rules.address ? rules.address.message : null"
            ...
            <a-row :gutter="16" v-if="formData.user_type != 'staff_members'">
                <a-col :xs="24" :sm="24" :md="24" :lg="24">
                    <a-form-item :label="$t('user.shipping_address')" name="shipping_address" :help="rules.shipping_address ? rules.shipping_address.message : null
                        " :validateStatus="rules.shipping_address ? 'error' : null">
                        <a-textarea v-model:value="formData.shipping_address" :placeholder="$t('common.placeholder_default_text', [
