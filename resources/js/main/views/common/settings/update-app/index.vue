<template>
    <a-card class="page-content-container mt-20 mb-20">
        <a-row class="mt-20">
            <a-col :span="24">
                <!-- <a-alert v-if="productStatus == 'fetching'" type="info" showIcon
                ...
                <a-alert v-if="productStatus == 'success'" type="success" showIcon
                    :message="$t('messages.product_is_upto_date')" />
                <a-alert v-if="productStatus == 'update_available'" type="warning" showIcon :message="$t('messages.new_app_version_avaialbe', [product.product.version])
                    " /> -->

                <div class="table-responsive mt-10">
                    <a-table :columns="tableColumns" :row-key="(record) => record.id" :data-source="appDetails"
                        :pagination="false" :showHeader="false">
                        <template #bodyCell="{ column, record }">
                            <template v-if="column.dataIndex == 'name'">
                                <a-typography-title :level="5" v-if="record.name == 'app_details'" strong>
